...
fun	:  ((AT state)* (party (COMMA party)* | TILDE) COLON funId=ID LPAR (vardec ( COMMA vardec)* )? RPAR SLPAR (assetdec ( COMMA assetdec)* )? SRPAR ( body | hobody) ) ;
...
//ESCAPED SEQUENCES
COMMENTS : 		LINECOMMENTS | BLOCKCOMMENTS;
LINECOMMENTS    : '//' (~('\n'|'\r'))* -> skip;
