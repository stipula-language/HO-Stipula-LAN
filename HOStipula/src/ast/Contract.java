	...
	public boolean runStatements(boolean valid, TypeInference tc, ArrayList<Statement> stms) {
		...
		for(Statement s : stms) {
			...
			if(s.getOperator().equals("FIELDUP")) {
				...
				if(fieldExprLeft && partyRight) {
					if(s.getFract()!=0) {
						globalParties.get(indexRight).moveAsset(globalParties.get(indexRight),((float)(s.getFract())*globalVars.get(indexLeft).getValue()));
					}
					else {
						...
						globalParties.get(indexRight).moveAsset(globalParties.get(indexRight),((float)(valFract)*globalVars.get(indexLeft).getValue()));
